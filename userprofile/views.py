...
class MyProfileAPIView(APIView):
    ...
    @swagger_auto_schema(
        ...
        "о себе.",
        responses={200: myprofile_swagger["response"]},
    )
    def get(self, request, *args, **kwargs):
        profile = request.user.profile
        serializer = ProfileSerializer(
            profile, context={"detail": True, "user": request.user, "me": True}
        )
    ...
    @swagger_auto_schema(
        ...
        responses={200: "Profile has been successfully changed.", 400: "Invalid data."},
    )
    def put(self, request, *args, **kwargs):
        ...
        serializer = ProfileSerializer(
            profile,
            data=request.data,
            context={"detail": False, "user": request.user, "me": True},
        )
...
class UserProfileAPIView(APIView):
    ...
    def get(self, request, slug, *args, **kwargs):
        ...
        serializer = ProfileSerializer(
            profile, context={"detail": True, "user": request.user, "me": False}
        )
        return Response(serializer.data, status=status.HTTP_200_OK)
